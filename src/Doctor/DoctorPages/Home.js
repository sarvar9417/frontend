...
export const Home = () => {
    ...
    const getOnline = useCallback(async () => {
        try {
            const fetch = await request('/api/section/on', 'GET', null, {
    ...
    const getOffline = useCallback(async () => {
        try {
            const fetch = await request('/api/section/off', 'GET', null, {
                Authorization: `Bearer ${auth.token}`
            })
            setOffline(fetch)
        } catch (e) {
                ...
                <article className="linkk blue mt-5" style={{ maxWidth: "700px", margin: "auto" }}>
                    ...
                    <div className="row">
                        <div className="col-12" style={{ marginTop: "31px", display: "flex" }}>
                            ...
                            />
                            <label className="labelss">F.I.O</label>
                            <button className="btn text-white" style={{ backgroundColor: "rgb(83, 158, 241)", width: "50%", marginLeft: "5%" }}>Kirish</button>
                        </div>
                    </div>
